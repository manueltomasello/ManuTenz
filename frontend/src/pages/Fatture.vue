...
<template>
  ...
  <div class="d-flex justify-content-between align-items-center mb-4">
      <h1 class="mb-0">Gestione Fatture</h1>
      <button @click="exportToExcel" class="btn btn-sm btn-outline-success">
        <i class="bi bi-file-earmark-excel"></i> Esporta in Excel
      </button>
    </div>

    <div class="row g-4">
      <div class="col-12 col-md-4">
        <h2 class="h5 mb-3">{{ selectedFattura ? 'Modifica' : 'Aggiungi' }} Fattura</h2>
        ...
        <form @submit.prevent="saveFattura" class="vstack gap-3">
          ...
          <div>
            ...
            <multiselect v-model="selectedIntervento":options="interventi"label="IntId"track-by="IntId"placeholder="Seleziona intervento":searchable="true"
            :close-on-select="true"required class="form-control">
          ...
          <div class="d-grid gap-2">
            ...
            <button v-if="selectedFattura" @click="resetForm" type="button" class="btn btn-secondary">
              Annulla modifica
            </button>
          </div>
        </form>
