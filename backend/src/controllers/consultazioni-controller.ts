...
export async function ConsCostoRisorsa(req: Request, res: Response) {
    ...
    try {
        const [results] = await conn.query(`
            ...
            LEFT JOIN risorsa r ON r.NomeRisorsa = CASE 
    ...
    try {
        const [results] = await conn.query(`
            WITH AggregatedOperatori AS (
                SELECT
                    id.IntId,
                    GROUP_CONCAT(CONCAT(o.NomeDip, ' ', o.CognDip) SEPARATOR ', ') AS OperatoriCoinvolti
                FROM interventi_dipendenti id
                JOIN operatore o ON id.IdDip = o.IdDip
                GROUP BY id.IntId
            )
            SELECT i.IntId AS InterventoID,
                ...
                COALESCE(c.Descrizione, 'Nessun Guasto') AS TipoGuasto,
...
export async function OrelavoratePerDip(req: Request, res: Response) {
    ...
    try {
         const [results] = await conn.query(`
            ...
            WHERE i.EsitoMan = 1
            GROUP BY o.IdDip, o.NomeDip, o.CognDip 
...
/**l'ho creata perchè mentre sviluppavo la getinterventiritardo mi sono reso conto che apportando una piccola 
 * modifica all'and della query potevo creare un middleware per una consultazione, se l'azienda richiederà lo sviluppo è già pronta la base **/
