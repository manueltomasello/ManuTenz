...
//l'ho dovuta creare perche CreateIntervento non è riutilizzabile in quanto non funziona pura ma middlware
  ...
  try {
    ...
    await conn.execute(
      `INSERT INTO interventi (
        ...
        noteIntervento, TipoGuastoId, OriginInt
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [newIntId, ManId, DataIntPrev, null, '08:00:00', '12:00:00', 0, 0, "intervento generato dal sistema", null, 1]
    );
  ...
  try {
    ...
    for (const man of manutenzioni) {
      ...
      const [interventiEffettuati] = await connection.promise().query<RowDataPacket[]>(
        `SELECT DataIntEff FROM interventi
         WHERE ManId = ?
...
export async function getInterventiCalendario(req: Request, res: Response) {
  const conn = await connection.promise().getConnection();
  try {const [results] = await conn.query<RowDataPacket[]>(
    `SELECT 
    i.IntId,
    i.ManId,
    i.NomeRisorsaInt,
    ...
    i.OraFine,
    CASE 
    WHEN i.ManId != 0 THEN m.DescMan
    ELSE i.NoteIntervento
    END AS DescMan,
    CASE 
    WHEN i.ManId != 0 THEN r1.ModMacc
    ELSE r2.ModMacc
    END AS ModMacc
    FROM interventi i
...
export async function getInterventiNonValidate(req: Request, res: Response) {
  ...
  try {
    const [results] = await conn.query<RowDataPacket[]>(
      `SELECT i.IntId, f.RagSoc FROM interventi i 
...
export async function getInterventiRitardo(req: Request, res: Response) {
  ...
  try {
    ...
    i.DataIntPrev,
    CASE
        WHEN i.ManId = 0 THEN i.NomeRisorsaInt
        ELSE r.NomeRisorsa
    END AS risorsa,
    CASE
        WHEN i.ManId = 0 THEN i.NoteIntervento
        ELSE m.DescMan
    END AS noteIntervento
FROM
    interventi i
LEFT JOIN
    manutenzioni m ON i.ManId = m.ManId
LEFT JOIN
    risorsa r ON m.maccIdMan = r.NomeRisorsa 
WHERE
    i.ValidataMan = 0
    AND i.DataIntPrev < CURDATE()
